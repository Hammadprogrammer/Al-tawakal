...
function buildReservationWhatsAppUrl(data: {
  name: string;
  phone: string;
  guests: string;
  date: string;
  ...
  const branch = ACTIVE_BRANCHES[0];
  const waNumber = (branch?.whatsapp || RESTAURANT_SETTINGS.whatsapp || '923485650906').replace(/\D/g, '');

  const text = `🔔 NEW TABLE RESERVATION — TAWAKAL BBQ
...
Branch: ${branch?.name || 'Tawakal Restaurant — Akhtar Colony'}
...
export function PublicReservationClient() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (typeof window !== 'undefined') {
      window.open(buildReservationWhatsAppUrl(formData), '_blank', 'noopener,noreferrer');
    }
          ...
          {submitted ? (
            <div className="py-12 text-center space-y-4">
              ...
              <p className="font-sans text-xs sm:text-sm text-[#9F9589] max-w-md mx-auto leading-relaxed">
                Thank you <strong className="text-[#F4EBDD]">{formData.name}</strong>. Your reservation details have been opened in WhatsApp — send the message and our reservation manager will confirm your table for {formData.guests} guests on {formData.date} on <strong className="text-[#C69A45]">{formData.phone}</strong>.
              </p>
              <div className="pt-4 flex flex-wrap items-center justify-center gap-3">
                <a
                  href={buildReservationWhatsAppUrl(formData)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-6 py-3 rounded-xl bg-[#25D366] hover:bg-[#20bd5a] text-white font-sans text-xs uppercase font-bold tracking-wider inline-flex items-center gap-2 shadow-lg transition-colors"
                >
