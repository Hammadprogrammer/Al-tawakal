...
import { ACTIVE_BRANCHES, getDeliveryAreasForBranch } from '@/data/static-data';
...
export function PublicCheckoutClient() {
  ...
  const branches: Branch[] = ACTIVE_BRANCHES;
  const [selectedBranchId, setSelectedBranchId] = useState<string>(branches[0]?.id || '');

  const [orderType, setOrderType] = useState<'DELIVERY' | 'PICKUP'>('DELIVERY');
  const [deliveryAreas, setDeliveryAreas] = useState<DeliveryArea[]>(
    getDeliveryAreasForBranch(branches[0]?.id || '')
  );
  const [selectedAreaId, setSelectedAreaId] = useState<string>(
    getDeliveryAreasForBranch(branches[0]?.id || '')[0]?.id || ''
  );
  ...
  // Refresh delivery areas whenever the selected branch changes
  useEffect(() => {
    if (!selectedBranchId) return;
    const areas = getDeliveryAreasForBranch(selectedBranchId);
    ...
    try {
      ...
      const order: LocalOrder = {
        ...
        branch: selectedBranch
          ? { name: selectedBranch.name, phone: selectedBranch.phone, whatsapp: selectedBranch.whatsapp }
          : null,
      ...
      try {
        launchCustomerWhatsApp({ ...order, area: areaName });
      } catch (waErr) {
