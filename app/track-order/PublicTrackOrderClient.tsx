...
import { findStoredOrder } from '@/lib/local-orders';
...
function TrackOrderContent() {
  ...
  const fetchTrackOrder = (num: string, ph: string) => {
    if (!num.trim() || !ph.trim()) {
    ...
    const found = findStoredOrder(num, ph);

    if (!found) {
      setOrder(null);
      setErrorMsg(
        'No order found on this device. Order records are stored on the device used to place the order — please contact us on WhatsApp for live status.'
      );
      toast.error('No order found on this device.');
      setLoading(false);
      return;
    }
