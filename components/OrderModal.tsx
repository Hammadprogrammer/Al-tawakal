...
import { generateOrderNumber, storeOrder, type LocalOrder } from '@/lib/local-orders';
import { ACTIVE_BRANCHES, RESTAURANT_SETTINGS } from '@/data/static-data';
...
export const OrderModal: React.FC<OrderModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const mainBranch = ACTIVE_BRANCHES[0];

      if (activeTab === 'order') {
        const isDelivery = Boolean(deliveryAddress);
        const unitPrice = initialPrice || 350;
        const orderSubtotal = unitPrice * quantity;
        const orderDeliveryFee = isDelivery ? Number(RESTAURANT_SETTINGS.delivery_fee || 150) : 0;
        ...
        const order: LocalOrder = {
          ...
          totalAmount: orderSubtotal + orderDeliveryFee,
          createdAt: new Date().toISOString(),
          branch: mainBranch
            ? { name: mainBranch.name, phone: mainBranch.phone, whatsapp: mainBranch.whatsapp }
            : null,
          orderItems: [
            {
              id: initialItemId || 'tawakal-special',
        ...
        storeOrder(order);
        launchCustomerWhatsApp(order);

        setOrderResult(order);
        setSubmitted(true);
      } else {
        // Table reservation — sent directly to the restaurant on WhatsApp
        const reservationNumber = generateOrderNumber().replace('TWK-', 'RES-');
        const reservationText = `🔔 NEW TABLE RESERVATION — TAWAKAL BBQ
...
Guests: ${guests}
Date: ${resDate}
Time: ${resTime}${deliveryNotes ? `\nSpecial Request: ${deliveryNotes}` : ''}

Please confirm my table reservation.`;

        const waNumber = (mainBranch?.whatsapp || RESTAURANT_SETTINGS.whatsapp || '923485650906').replace(
          /\D/g,
          ''
        );
        ...
        if (typeof window !== 'undefined') {
          window.open(
            `https://wa.me/${waNumber}?text=${encodeURIComponent(reservationText)}`,
            '_blank',
            'noopener,noreferrer'
          );
        }
          ...
          {!submitted ? (
            ...
            <div className="py-6 text-center flex flex-col items-center">
              ...
              <p className="font-sans text-xs text-[#B8B0A5] font-normal max-w-sm mb-6 leading-relaxed">
                Thank you <span className="text-[#D96A2B] font-semibold">{guestName}</span>. Your order details have been opened in WhatsApp — just hit send and our team will prepare your charcoal feast immediately.
              </p>
