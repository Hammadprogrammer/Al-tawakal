...
import { MENU_CATEGORIES, MENU_ITEMS } from '@/data/static-data';
...
const categories = MENU_CATEGORIES.map((cat) => ({ id: cat.slug, label: cat.name }));
...
const menuItems = MENU_ITEMS.map((item) => ({
  ...
  price: `Rs. ${item.price}`,
  rawPrice: item.price,
  category: item.category?.slug || 'signature-bbq',
  image: item.image,
  isPopular: item.isPopular,
  isChefSpecial: item.isFeatured,
  tags: item.isFeatured ? ['Best Seller', 'Live Fire'] : ['Fresh Charcoal'],
}));

export const Menu: React.FC<MenuProps> = ({ onOrderDish }) => {
  const [activeCategory, setActiveCategory] = useState<string>(
    categories.find((cat) => cat.id === 'signature-bbq')?.id || categories[0]?.id || 'signature-bbq'
  );
